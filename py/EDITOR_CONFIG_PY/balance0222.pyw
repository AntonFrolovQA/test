...
def get_UI():
    ...
    layout = [
        ...
        [sg.Checkbox("Simple Recipies", key='simple'), sg.Text('(Крафт за 1 бревно)', text_color="grey")],
        [sg.Checkbox("Easy Death", change_submits = True, enable_events=True, key='fast'), sg.Text('(Быстро сфидить: поднять камень и выпить)', text_color="grey")],

        [sg.Text("Preset")], 
        [sg.Button("Soft", key='-PUSSY-'), sg.Button("Hard", key='-KILLER-'), sg.Text(""), sg.Text(stat_key(21)), sg.Input(default_text=stats[stat_key(21)][0], size=(5, 10), key="22"),
       
        sg.Input(default_text=stats[stat_key(22)][0],
        change_submits=True,
        readonly=False,
        do_not_clear=True,
        enable_events=True,
    ...
    window = sg.Window('\U0001f638 balance v0.222', layout, grab_anywhere=1)
    ...
    while True:
        ...
        if event == "OK":
            ...
            return Amounts, createSimpleRecepies, fastDeath
...
def get_useItems(root, amounts, fastDeath):
...
def xml_parseBalance(path):
    ...
    root = tree.getroot()
    amounts, createSimpleRecepies, fastDeath = get_UI()
    ...
    for _ in useList:
        get_useItems(root, amounts, fastDeath)

    # with open(path, 'wb') as f:
    tree.write(path, encoding="utf-8")
    return createSimpleRecepies, fastDeath
...
def main():
    createSimpleRecepies, fastDeath = xml_parseBalance(balance_path)
    if createSimpleRecepies == 1:
        get_MetaData(balance_path)
    if fastDeath == 1:
        get_MetaData01(balance_path)
